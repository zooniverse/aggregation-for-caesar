...
@tool_wrapper
@subtask_wrapper
def point_extractor_temporal(classification, **kwargs):
    ...
    for annotation in classification['annotations']:
        ...
        for _, value in enumerate(annotation['value']):
            ...
            if ('x' in value) and ('y' in value) and ('displayTimeStamp' in value):
                extract[frame].setdefault('{0}_x'.format(key), []).append(value['x'])
                extract[frame].setdefault('{0}_y'.format(key), []).append(value['y'])
                try:
                    timestamp = time.strptime(value['displayTimeStamp'], '%H:%M:%S')
                except ValueError:
                    timestamp = time.strptime(value['displayTimeStamp'], '%M:%S')
