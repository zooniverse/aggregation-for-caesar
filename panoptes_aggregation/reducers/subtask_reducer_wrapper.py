        ...
        try:
            if details_functions is not None:
                ...
                if all(classifier_versions == version.parse('1.0')):
                    ...
                    keys_details = ['{0}_details'.format(k) for k in details_functions_v1.keys()]
                    keys_labels = ['{0}_cluster_labels'.format(k) for k in details_functions_v1.keys()]
                    keys_clusters = ['{0}_clusters_details'.format(k) for k in details_functions_v1.keys()]
                    ...
                    for frame_key, _frame in output.items():
                        for kd, kl, kc, df in zip(keys_details, keys_labels, keys_clusters, details_functions_v1.keys()):
                                ...
                                for label in np.unique(cluster_labels):
                                    if label > -1:
                                        ...
                                        for ddx, detail in enumerate(detail_cluster.T):
                                            if details_functions_v1[df][ddx] is None:
                                                detail_reduced.append('No reducer for this subtask type')
                                            else:
                                                reducer = reducers.reducers[details_functions_v1[df][ddx]]
                    ...
                    user_ids_per_subtask = defaultdict(lambda: defaultdict(list))
                    for edx, extract in enumerate(data_in):
                        for frame_key, frame_in in extract.items():
                            # will convert 1.0 subtask extract to 2.0 subtask extract if needed
                            # could technically add flag to output v1.0 only in v2.0 flattened
                            # subtask format
                            frame = details_extract_flatten(frame_in)
                            for k in details_functions_v2.keys():
                                output[frame_key].setdefault(k, [])
                                if k in frame:
                                    output[frame_key][k] += frame[k]
                                    user_ids_per_subtask[frame_key][k] += [user_id[edx]] * len(frame[k])
                            ...
                            for subtask in subtasks:
                                ...
                                for label in np.unique(cluster_labels):
                                    if label > -1:
                                        ...
                                        user_id_cluster = user_id_array[cdx]
                                        reducer = reducers.reducers[details_functions_v2[subtask]]
