...
class TestClusterLines(unittest.TestCase):
    ...
    def test_process_data_by_line(self):
        result = process_data(extracted_data, process_by_line=True)
        self.assertDictEqual(dict(result), processed_data_by_line)
    ...
    def test_poly_line_text_reducer_by_line(self):
        result = poly_line_text_reducer(extracted_data, **self.kwargs_by_line)
        self.assertDictEqual(dict(result), reduced_data_by_line)
    ...
    def test_poly_line_text_reducer_request_by_line(self):
        app = flask.Flask(__name__)
        request_kwargs = {
            'data': json.dumps(extract_in_data(extracted_data)),
            'content_type': 'application/json'
        }
        url_params = '?{0}'.format(urllib.parse.urlencode(self.kwargs_by_line))
        with app.test_request_context(url_params, **request_kwargs):
            result = poly_line_text_reducer(flask.request)
            self.assertDictEqual(dict(result), reduced_data_by_line)
