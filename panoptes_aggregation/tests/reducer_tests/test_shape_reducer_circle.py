from panoptes_aggregation.reducers.shape_reducer_dbscan import process_data as process_data_dbscan, shape_reducer_dbscan
from panoptes_aggregation.reducers.shape_reducer_hdbscan import process_data as process_data_hdbscan, shape_reducer_hdbscan
from panoptes_aggregation.reducers.shape_reducer_optics import process_data as process_data_optics, shape_reducer_optics
from .base_test_class import ReducerTest, CollabTest
import copy

extracted_data = [
    {
        'frame0': {
            'T0_tool0_x': [0.0, 100.0],
            'T0_tool0_y': [0.0, 100.0],
            'T0_tool0_r': [50.0, 10.0]
        },
        'frame1': {
            'T0_tool1_x': [50.0],
            'T0_tool1_y': [50.0],
            'T0_tool1_r': [50.0]
        }
    },
    {
        'frame0': {
            'T0_tool0_x': [0.0, 100.0],
            'T0_tool0_y': [0.0, 100.0],
            'T0_tool0_r': [50.0, 10.0],
            'T0_tool1_x': [0.0, 100.0],
            'T0_tool1_y': [100.0, 0.0],
            'T0_tool1_r': [10.0, 50.0]
        }
    },
    {
        'frame1': {
            'T0_tool1_x': [50.0],
            'T0_tool1_y': [50.0],
            'T0_tool1_r': [50.0]
        }
    },
    {
        'frame0': {
            'T0_tool1_x': [0.0, 100.0],
            'T0_tool1_y': [100.0, 0.0],
            'T0_tool1_r': [10.0, 50.0]
        },
        'frame1': {
            'T0_tool0_x': [20.0],
            'T0_tool0_y': [20.0],
            'T0_tool0_r': [20.0]
        }
    },
    {}
]

kwargs_extra_data = {
    'user_id': [
        1,
        2,
        3,
        4,
        5
    ]
}

processed_data = {
    'shape': 'circle',
    'n_classifications': 5,
    'symmetric': False,
    'frame0': {
        'T0_tool0': [
            (0.0, 0.0, 50.0),
            (100.0, 100.0, 10.0),
            (0.0, 0.0, 50.00),
            (100.0, 100.0, 10.0)
        ],
        'T0_tool1': [
            (0.0, 100.0, 10.0),
            (100.0, 0.0, 50.0),
            (0.0, 100.0, 10.0),
            (100.0, 0.0, 50.0)
        ]
    },
    'frame1': {
        'T0_tool0': [
            (20.0, 20.0, 20.0)
        ],
        'T0_tool1': [
            (50.0, 50.0, 50.0),
            (50.0, 50.0, 50.0)
        ]
    }
}

reduced_data = {
    'frame0': {
        'T0_tool0_circle_x': [0.0, 100.0, 0.0, 100.0],
        'T0_tool0_circle_y': [0.0, 100.0, 0.0, 100.0],
        'T0_tool0_circle_r': [50.0, 10.0, 50.0, 10.0],
        'T0_tool0_cluster_labels': [0, 1, 0, 1],
        'T0_tool0_clusters_count': [2, 2],
        'T0_tool0_clusters_x': [0.0, 100.0],
        'T0_tool0_clusters_y': [0.0, 100.0],
        'T0_tool0_clusters_r': [50.0, 10.0],
        'T0_tool0_n_classifications': [5, 5],
        'T0_tool0_shape': ['circle', 'circle'],
        'T0_tool1_circle_x': [0.0, 100.0, 0.0, 100.0],
        'T0_tool1_circle_y': [100.0, 0.0, 100.0, 0.0],
        'T0_tool1_circle_r': [10.0, 50.0, 10.0, 50.0],
        'T0_tool1_cluster_labels': [0, 1, 0, 1],
        'T0_tool1_clusters_count': [2, 2],
        'T0_tool1_clusters_x': [0.0, 100.0],
        'T0_tool1_clusters_y': [100.0, 0.0],
        'T0_tool1_clusters_r': [10.0, 50.0],
        'T0_tool1_n_classifications': [5, 5],
        'T0_tool1_shape': ['circle', 'circle']
    },
    'frame1': {
        'T0_tool0_circle_x': [20.0],
        'T0_tool0_circle_y': [20.0],
        'T0_tool0_circle_r': [20.0],
        'T0_tool0_cluster_labels': [-1],
        'T0_tool1_circle_x': [50.0, 50.0],
        'T0_tool1_circle_y': [50.0, 50.0],
        'T0_tool1_circle_r': [50.0, 50.0],
        'T0_tool1_cluster_labels': [0, 0],
        'T0_tool1_clusters_count': [2],
        'T0_tool1_clusters_x': [50.0],
        'T0_tool1_clusters_y': [50.0],
        'T0_tool1_clusters_r': [50.0],
        'T0_tool1_n_classifications': [5],
        'T0_tool1_shape': ['circle']
    }
}

TestShapeReducerCircle = ReducerTest(
    shape_reducer_dbscan,
    process_data_dbscan,
    extracted_data,
    processed_data,
    reduced_data,
    'Test shape circle reducer with DBSCAN',
    network_kwargs=kwargs_extra_data,
    pkwargs={'shape': 'circle'},
    kwargs={
        'eps': 5,
        'min_samples': 2
    },
    test_name='TestShapeReducerCircle'
)

TestShapeReducerCircleOptics = ReducerTest(
    shape_reducer_optics,
    process_data_optics,
    extracted_data,
    processed_data,
    reduced_data,
    'Test shape circle reducer with OPTICS',
    network_kwargs=kwargs_extra_data,
    pkwargs={'shape': 'circle'},
    kwargs={
        'min_samples': 2
    },
    test_name='TestShapeReducerCircleOptics'
)

reduced_data_hdbscan = copy.deepcopy(reduced_data)
reduced_data_hdbscan['frame0']['T0_tool0_cluster_probabilities'] = [1.0, 1.0, 1.0, 1.0]
reduced_data_hdbscan['frame0']['T0_tool1_cluster_probabilities'] = [1.0, 1.0, 1.0, 1.0]
reduced_data_hdbscan['frame1']['T0_tool0_cluster_probabilities'] = [0.0]
reduced_data_hdbscan['frame1']['T0_tool1_cluster_probabilities'] = [1.0, 1.0]

TestShapeReducerCircleHdbscan = ReducerTest(
    shape_reducer_hdbscan,
    process_data_hdbscan,
    extracted_data,
    processed_data,
    reduced_data_hdbscan,
    'Test shape circle reducer with HDBSCAN',
    network_kwargs=kwargs_extra_data,
    pkwargs={'shape': 'circle'},
    kwargs={
        'min_cluster_size': 2,
        'min_samples': 1,
        'allow_single_cluster': True
    },
    test_name='TestShapeReducerCircleHdbscan'
)

data_collab_true = [
    {
        'min_threshold': 0,
        'threshold': 0.4,
        'stepKey': 'S0',
        'taskIndex': 0,
        'taskKey': 'T0',
        'taskType': 'circle',
        'toolIndex': 0,
        'frame': 0,
        'markID': 'consensus_0',
        'toolType': 'freehandLine',
        'pathX': 0.0,
        'pathY': 0.0,
        'pathR': 50.0
    },
    {
        'min_threshold': 0,
        'threshold': 0.4,
        'stepKey': 'S0',
        'taskIndex': 0,
        'taskKey': 'T0',
        'taskType': 'circle',
        'toolIndex': 0,
        'frame': 0,
        'markID': 'consensus_1',
        'toolType': 'freehandLine',
        'pathX': 100.0,
        'pathY': 100.0,
        'pathR': 10.0
    },
    {
        'min_threshold': 0,
        'threshold': 0.4,
        'stepKey': 'S0',
        'taskIndex': 0,
        'taskKey': 'T0',
        'taskType': 'circle',
        'toolIndex': 1,
        'frame': 0,
        'markID': 'consensus_0',
        'toolType': 'freehandLine',
        'pathX': 0.0,
        'pathY': 100.0,
        'pathR': 10.0
    },
    {
        'min_threshold': 0,
        'threshold': 0.4,
        'stepKey': 'S0',
        'taskIndex': 0,
        'taskKey': 'T0',
        'taskType': 'circle',
        'toolIndex': 1,
        'frame': 0,
        'markID': 'consensus_1',
        'toolType': 'freehandLine',
        'pathX': 100.0,
        'pathY': 0.0,
        'pathR': 50.0
    },
    {
        'min_threshold': 0,
        'threshold': 0.4,
        'stepKey': 'S0',
        'taskIndex': 0,
        'taskKey': 'T0',
        'taskType': 'circle',
        'toolIndex': 1,
        'frame': 1,
        'markID': 'consensus_0',
        'toolType': 'freehandLine',
        'pathX': 50.0,
        'pathY': 50.0,
        'pathR': 50.0
    }
]

TestShapeReducerCircleCollabTrue = CollabTest(
    shape_reducer_optics,
    extracted_data,
    data_collab_true,
    'Test data is correct when collab: True',
    kwargs={
        'min_samples': 2,
        'shape': 'circle',
        'collab': True
    },
    network_kwargs=kwargs_extra_data,
    test_name='TestShapeReducerCircleCollabTrue'
)
